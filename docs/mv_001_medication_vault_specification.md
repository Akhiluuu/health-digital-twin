...
| :--- | :--- |
| **A simple drug encyclopedia.** We do not expect users to read dry chemical monographs. | **An AI-Supported Medication Workspace.** Personal Health Assistant translates pharmacy sheets into plain English. |
...
VitalHealth PHOS
 └── Medication Vault Module
      ...
      │    │    └── AI Assistant Workspace (Personal Health Assistant Chat History on Diabetes)
...
- **Deep Linking Matrix**:
  ...
  - `vitalhealth://assistant/chat?context=medication&id={medication_id}` -> Opens Personal Health Assistant with a preloaded prompt focusing on the specified medication.
...
sequenceDiagram
    ...
    participant A as Personal Health Assistant
...
- **Proposed**: Generated by Personal Health Assistant after analyzing a uploaded clinical note. Prompt: *"Personal Health Assistant detected a new treatment recommendation for Hypertension. Review and activate?"*
...
```
+-------------------------------------------------------------+
| [Profile Switcher: Self v]                      [Personal Health Assistant]   |
|                                                             |
...
### UI Components
- **Top Bar**: Profile switcher dropdown (Self, Father, Son), search icon, and Personal Health Assistant mini-button.
...
- **Treatment Cards**: Large containers that group:
  ...
  - A summary card containing AI-generated advice from Personal Health Assistant (e.g., *"Your blood pressure has trended down since starting Lisinopril. Personal Health Assistant recommends bringing this chart to your follow-up on Aug 12."*).
...
- **Personal Health Assistant Insight Panel**: A collapsed card that expands to show common side effects in plain English.
...
## 17. AI Integration (Personal Health Assistant)

Personal Health Assistant is integrated directly into the workspace to help users understand their medications.

```
+-------------------------------------------------------------+
| [Assistant Logo] Ask Personal Health Assistant about Metformin                    |
|                                                             |
|  Common questions:                                          |
|  +-------------------------------------------------------+  |
...
### Design Guidelines
- **Contextual Access**: A Personal Health Assistant quick-help card is embedded inside the detail screen of every medication.
- **Clinical Review Support**: When a drug reaches the `Review Due` phase, Personal Health Assistant prepares a diagnostic survey for the patient:
...
- **Plain-Language Translations**: When parsing FDA drug monographs, Personal Health Assistant translates clinical terms into simple language. For example, instead of *"May cause transient dyspepsia,"* Personal Health Assistant shows *"May cause temporary stomach upset. Taking it with food helps."*
- **Interaction Checker**: Run automatically in the background when a new drug is added. If a conflict is detected, a warning appears at the top of the Add Medication flow: *"Personal Health Assistant detected a moderate interaction between your new medication and your active Lisinopril. We recommend confirming this with your pharmacist."*
...
|  [x] Clinical Review Survey Responses (Personal Health Assistant)            |
...
- **Clinical Review Passport**: A dedicated report generated for upcoming reviews. It includes the active drug list, 30-day compliance logs, related Digital Twin telemetry (e.g., blood pressure/glucose patterns), and patient symptom entries collected by Personal Health Assistant.
