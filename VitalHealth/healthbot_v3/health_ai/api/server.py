...
async def _handle_medicine_query(query: str, q: str, medicines) -> str:
    ...
    def _first_sentence(text: str) -> str:
        """Return up to 2 sentences from the LLM response for richer medicine info."""
        clean = _clean_text(text)
        if not clean:
            return ""
        parts = re.split(r'(?<=[.!?])\s+', clean)
        # Return first 2 sentences joined, or just 1 if only 1 exists
        if len(parts) >= 2:
    ...
    async def _explain_medicine(name: str, dose: Optional[str], api_info: Optional[dict]) -> str:
        ...
        prompt = f"""You are Dr. Aria, a warm and knowledgeable health assistant.
...
Sentence 1: What this medicine does and how it works in the body (be specific).
Sentence 2: What condition or symptom it is commonly used to treat, and one key thing to know about taking it.

Rules:
- No bullets, no headings, no markdown symbols, no numbered lists.
- Plain readable English. Friendly and informative.
- Do not start with the medicine name."""
        try:
            raw = await _ask_llm(prompt, max_tokens=120)
        except Exception:
            raw = ""
...
- Flag critical symptoms with 🔴""", max_tokens=220)
...
- End with: "Your doctor is the best person to confirm this. 💙" """, max_tokens=200)
...
# ── BioGears AI Endpoints ─────────────────────────────────────────────────────

class BiogearsVitalsIn(BaseModel):
    ...
    exercise_level:    Optional[float] = None


class BiogearsAnomalyIn(BaseModel):
    label:        str
    severity:     str
    value:        float
    normal_range: str
...
class BiogearsInsightsRequest(BaseModel):
    vitals:               BiogearsVitalsIn
    anomalies:            List[BiogearsAnomalyIn] = []
...
class BiogearsInsightsResponse(BaseModel):
    insights_text: str
    bullet_points: List[str]


class BiogearsQueryRequest(BaseModel):
...
class BiogearsQueryResponse(BaseModel):
    response: str


def _build_biogears_context(
    vitals: BiogearsVitalsIn,
    anomalies: List[BiogearsAnomalyIn],
    has_anomaly: bool,
    interaction_warnings: List[str],
    data_gap_warning: Optional[str],
    events_summary: Optional[str],
) -> str:
    lines = ["=== BioGears Simulation Results ===", "\n[Vitals Recorded]"]
    v = vitals
    if v.heart_rate is not None:      lines.append(f"  Heart Rate: {v.heart_rate} bpm")
    if v.blood_pressure:              lines.append(f"  Blood Pressure: {v.blood_pressure} mmHg")
    if v.spo2 is not None:            lines.append(f"  SpO2: {v.spo2}%")
    if v.glucose is not None:         lines.append(f"  Blood Glucose: {v.glucose} mg/dL")
    if v.respiration is not None:     lines.append(f"  Respiration Rate: {v.respiration} breaths/min")
    if v.core_temperature is not None: lines.append(f"  Core Temperature: {v.core_temperature}°C")
    if v.cardiac_output is not None:  lines.append(f"  Cardiac Output: {v.cardiac_output} L/min")
    if v.map is not None:             lines.append(f"  Mean Arterial Pressure: {v.map} mmHg")
    if v.stroke_volume is not None:   lines.append(f"  Stroke Volume: {v.stroke_volume} mL")
    if v.arterial_ph is not None:     lines.append(f"  Arterial pH: {v.arterial_ph}")
    if has_anomaly and anomalies:
        lines.append("\n[Anomalies Detected]")
        for a in anomalies:
            lines.append(f"  !! {a.label}: {a.value} (Severity: {a.severity}, Normal: {a.normal_range})")
    else:
    ...
    if interaction_warnings:
        ...
        for w in interaction_warnings:
            lines.append(f"  {w}")
    if data_gap_warning:
        lines.append(f"\n[Data Gap Notice]: {data_gap_warning}")
    if events_summary:
        lines.append(f"\n[Logged Events This Session]: {events_summary}")
...
@app.post("/biogears-insights", response_model=BiogearsInsightsResponse, tags=["BioGears AI"])
async def biogears_insights(request: BiogearsInsightsRequest):
    """Generate AI narrative insights from a BioGears simulation result."""
    if _llm is None or not _llm._loaded:
        raise HTTPException(status_code=503, detail="LLM not loaded.")
    context = _build_biogears_context(
        vitals=request.vitals, anomalies=request.anomalies,
        has_anomaly=request.has_anomaly, interaction_warnings=request.interaction_warnings,
        data_gap_warning=request.data_gap_warning, events_summary=request.events_summary,
    )
    system_prompt = get_system_prompt("biogears_insights")
    user_prompt   = (
        "Analyse the following BioGears simulation results and generate "
        "personalised health insights for the patient.\n\n" + context
    )
    log.info("BioGears insights — generating narrative…")
    loop = asyncio.get_running_loop()
    try:
        insights_text = await loop.run_in_executor(
            None, lambda: _llm.generate(system_prompt, user_prompt, max_tokens=600)
        )
    except GenerationError as e:
        raise HTTPException(status_code=500, detail=str(e))
    return BiogearsInsightsResponse(
        insights_text=insights_text,
        bullet_points=_extract_bullet_points(insights_text),
    )


@app.post("/biogears-query", response_model=BiogearsQueryResponse, tags=["BioGears AI"])
async def biogears_query(request: BiogearsQueryRequest):
    """Answer a natural-language question about BioGears simulation results."""
    if _llm is None or not _llm._loaded:
        raise HTTPException(status_code=503, detail="LLM not loaded.")
    context = _build_biogears_context(
        vitals=request.vitals, anomalies=request.anomalies,
        has_anomaly=request.has_anomaly, interaction_warnings=request.interaction_warnings,
        data_gap_warning=None, events_summary=request.events_summary,
    )
    system_prompt = get_system_prompt("biogears_query")
    history_block = ""
    if request.history:
        trimmed = request.history[-6:]
        pairs   = [(trimmed[i], trimmed[i+1]) for i in range(0, len(trimmed) - 1, 2)]
        history_block = "\n\n[Previous conversation]\n" + "\n".join(
            f"User: {u}\nDr. Aria: {a}" for u, a in pairs
        )
    user_prompt = f"{context}{history_block}\n\nPatient question: {request.query}"
    log.info(f"BioGears query: {request.query!r}")
    loop = asyncio.get_running_loop()
    try:
        response = await loop.run_in_executor(
            None, lambda: _llm.generate(system_prompt, user_prompt, max_tokens=300)
        )
    except GenerationError as e:
        raise HTTPException(status_code=500, detail=str(e))
    return BiogearsQueryResponse(response=apply_safety_layer(response, request.query))
